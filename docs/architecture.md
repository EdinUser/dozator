...
Формите с табове използват поле `mode`. Скритите панели са disabled, за да не попадат неактивни полета във `FormData`. При restore от QR/history/templates първо се избира правилният `mode`, после се попълват стойностите и изчислението се изпълнява наново.
...
- Калкулатори с табове пазят `mode`, така че dilution и infusion се възстановяват в правилния режим.
- In-memory drafts пазят стойности при преминаване от калкулатор към неговата документация и обратно; `Ново изчисление` изчиства текущата форма без да връща към началния екран.
...
- Няма patient-specific dose recommendation logic; теглото се използва само като числов множител при инфузионни изчисления по kg.
