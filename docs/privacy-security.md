...
Разрешени са само whitelisted полета, нужни за преизчисляване на конкретния калкулатор. Полето `mode` е разрешено за калкулатори с табове, за да се възстанови правилният режим. Числовото тегло в `kg` е разрешено само като изчислително поле за инфузионни изчисления по kg; не добавяйте име, ЕГН или други идентификатори към него. Allowlist-ът е в `src/share/share-link.js`.
...
- история;
- шаблони.
- незавършени стойности в паметта на текущата browser сесия при отваряне на документацията.
